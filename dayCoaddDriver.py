...
from lsst.pipe.tasks.makeDayCoaddTempExp import MakeDayCoaddTempExpTask
...
from lsst.pipe.drivers.utils import getDataRef, NullSelectImagesTask, TractDataIdContainer, DayTractDataIdContainer


class DayCoaddDriverConfig(Config):
    coaddName = Field(dtype=str, default="day", doc="Name for coadd")
    select = ConfigurableField(
        target=WcsSelectImagesTask, doc="Select images to process")
    makeDayCoaddTempExp = ConfigurableField(
        target=MakeDayCoaddTempExpTask, doc="Warp images to sky")
    ...
    def setDefaults(self):
        self.makeDayCoaddTempExp.select.retarget(NullSelectImagesTask)
        self.assembleCoadd.select.retarget(NullSelectImagesTask)
        self.makeDayCoaddTempExp.doOverwrite = False
        self.assembleCoadd.doWrite = False
        self.assembleCoadd.doMatchBackgrounds = False
        self.makeDayCoaddTempExp.bgSubtracted = True
        self.assembleCoadd.badMaskPlanes = [
            'BAD', 'EDGE', 'SAT', 'INTRP', 'NO_DATA']

    def validate(self):
        if self.makeDayCoaddTempExp.coaddName != self.coaddName:
            raise RuntimeError(
...
class DayCoaddDriverTaskRunner(CoaddTaskRunner):
...
class DayCoaddDriverTask(BatchPoolTask):
    ConfigClass = DayCoaddDriverConfig
    _DefaultName = "dayCoaddDriver"
    RunnerClass = DayCoaddDriverTaskRunner

    def __init__(self, *args, **kwargs):
        BatchPoolTask.__init__(self, *args, **kwargs)
        self.makeSubtask("select")
        self.makeSubtask("makeDayCoaddTempExp")
    ...
    def _makeArgumentParser(cls, **kwargs):
        ...
        parser.add_id_argument("--id", "dayCoadd", help="data ID, e.g. --id tract=12345 patch=1,2",
    ...
    def run(self, tractPatchRefList, butler, selectIdList=[]):
        ...
        tractIdList = []
        print ("tractPatchRefList : ",tractPatchRefList)
        ...
        print("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAHHHHHHHHHHHHHHH")
    ...
    def warp(self, cache, patchId, selectDataList):
        ...
        with self.logOperation("warping %s" % (patchRef.dataId,), catch=True):
            self.makeDayCoaddTempExp.run(patchRef, selectDataList)
